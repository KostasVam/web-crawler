...
**A:** To combine "check if exists" and "add" into a single atomic operation. If I had separate `has()` and `add()` methods, two workers could both call `has()`, both get `false`, and both call `add()` — race condition, both crawl the same URL. By returning whether the URL was newly added, the caller gets both pieces of information from one atomic call. In Redis this maps directly to `SADD`, which returns 1 or 0. In Java, `ConcurrentHashMap.putIfAbsent()` follows the same pattern — returns `null` if it was inserted, or the existing value if not.

⚠ Watch out: the interviewer might ask "what if you exposed both methods anyway?" Answer: you *can*, but atomic-by-default is safer — easier to use correctly than to use wrong. That's why the `VisitedStore` interface only has `add()` — no `has()` at all.
